...
export class SignalServerStore {
  ...
  getPreKeyBundle(userId: any) {
    if (!existsSync(path.join(__dirname, "../keys/", userId))) {
      throw new Error("No keys present!");
      //this.registerNewPreKeyBundle(userId, preKeyBundle)
    }
    let data = readFileSync(path.join(__dirname, "../keys/", userId))
    if (!data) {
      throw new Error("No keys present!");
      //this.registerNewPreKeyBundle(userId, preKeyBundle)
    }
    ...
    try {
      preKeyBundle = JSON.parse(data.toString())
    } catch (error) {
      console.error("Error in parsing the preKeyBundle");
      console.error(data);
    }
